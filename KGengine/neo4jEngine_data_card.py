...
class neo4jEngineDataCard:
    ...
    def create_node(self, data):
        with self.driver.session() as session:
            session.execute_write(self.create_node_from_json_data_card, data)
    ...
    def create_node_from_json_data_card(self, tx, data, leaf=None):
        if leaf is not None:
                ...
                for key, value in data.items():
                    ...
                    if isinstance(value, str) or isinstance(value, int) \
                        ...
                        for item in value:
                            if isinstance(item, dict):
                                ...
                                tx.run(
                                    ...
                                    f" CREATE (a)-[r:{key}]->(b)")
                                self.create_node_from_json_data_card(tx, item, leaf=node)
                        ...
                        tx.run(f"MATCH (a) WHERE id(a) = {leaf.element_id} MATCH (b) WHERE id(b) = {node.element_id}"
                               f" CREATE (a)-[r:{key}]->(b)")
                        self.create_node_from_json_data_card(tx, value, leaf=node)
...
if __name__ == '__main__':
    ...
    file = open('/Users/luoyu/PycharmProjects/DataModelCard/card/data/sample_card_template.json')

    data = json.load(file)

    run = neo4jEngineDataCard()
